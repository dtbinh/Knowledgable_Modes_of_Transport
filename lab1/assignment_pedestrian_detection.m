...
all_test_hog_pca  = apply_PCA(pca_hog, all_test_hog);
...
plot_ROC(test_labels, dval_hog_gmm, 'b', 'DisplayName', 'HOG/GMM');
...
for frame_id = 1:T
    ...
    use_svm = 1;
    if use_svm == 1
        decision_val = evaluate_SVM(svm_hog, frame_test_hog);
        decision_thresh = 0.99;
    else
        frame_test_hog_pca  = apply_PCA(pca_hog, frame_test_hog);
        decision_val = evaluate_GMM(gmm_hog, frame_test_hog_pca);
        decision_thresh = 0.99;
    end
